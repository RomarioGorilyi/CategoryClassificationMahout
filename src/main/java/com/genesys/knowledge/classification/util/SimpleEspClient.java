...
public class SimpleEspClient {
  ...
  public static Request3rdServer makeRequest3rdServer(String service, String method, KeyValueCollection parameters,
    ...
    KeyValueCollection keyValueCollection = asKeyValueCollection("AppName", "UnitTestClient", "AppType", "UnitTestAppType",
      "Service", service, "Method", method, "Parameters", parameters);
  ...
  public static KeyValueCollection asKeyValueCollection(Object... keyValues) {
    ...
    if (keyValues.length % 2 != 0) {
      throw new IllegalArgumentException("asKeyValueCollection requires an even number of parameters: " + Arrays.toString(keyValues));
    }
   ...
   * <p/>
   * Starting from PSDK 8.1.100, we need SSLContext in case of auto-upgrade or full TLS. For UCS 8.1.3: We should prepare SSLContext
   * from configuration (PSDK TLSHelpers from apptemplate)<br/> For UCS 8.1.2: keep backward compatibility: always trust server certificate!
  ...
  public synchronized void open() throws InterruptedException, ProtocolException, IllegalStateException {
    if (!isConnected()) {
      ...
      if (endpoint == null) {
        ...
        endpoint = new Endpoint(name, ucsHost, ucsPort, config, false, getClientSSLContext(), null);
      }
    ...
    if (connection.getState() != ChannelState.Opened) {
      fail("Could not connect to UCS ('" + ucsHost + ":" + ucsPort + "') within " + connectTimeout + "ms, Status=" + connection.getState());
    }
  }
  ...
  public synchronized void close() throws ProtocolException, IllegalStateException, InterruptedException {
    ...
    if (connection.getState() != ChannelState.Closed) {
      fail("Could not disconnect from UCS ('" + ucsHost + ":" + ucsPort + "')"
        + " within " + connectTimeout + "ms, Status=" + connection.getState());
    }
  }
